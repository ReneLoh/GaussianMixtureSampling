...
vector <double> OBABO_simu(const params param0, const size_t N, const double h, const double T, const double gamma, 
									const vector <double>& Xdata,	const double sig1, const double sig2, const double a1, const double a2, const size_t n_meas);
vector <double> OBABO_simu_MH(const params param0, const size_t N, const double h, const double T, const double gamma, const vector <double>& Xdata, const double sig1, 
										const double sig2, const double a1, const double a2, const size_t L, const bool SF, const size_t n_meas);
...
int method = atoi(argv[1]);   // must be 1 or 2
...
//	bin_ctr = OBABO_simu(param0, N_0, h, T, gamma, xrange, yrange, nrbins, Xdata, sig1, sig2, a1, a2, biasx);
	Tconfigs = OBABO_simu(param0, N_0, h, T, gamma, Xdata, sig1, sig2, a1, a2, n_meas);
	label = "OBABO";
}
else if(method==2){
	Tconfigs = OBABO_simu_MH(param0, N, h, T, gamma, Xdata, sig1, sig2, a1, a2, L, SF, n_meas);
...
stream << std::fixed << std::setprecision(3) << h;
ofstream file {"Tconf_" + label + "_h"+stream.str()};
...
vector <double> OBABO_simu(const params param0, const size_t N, const double h, const double T, const double gamma, 
   ...
   params theta = param0; 
	forces force = get_noisy_force_GM(T, theta, sig1, sig2, a1, a2, Xdata);    // HERE FORCES!!!!
   ...
   double a = exp(-1*gamma*h);      
	...
	for(size_t i=1; i<N; ++i){
		...
		force = get_noisy_force_GM(T, theta, sig1, sig2, a1, a2, Xdata);   // HERE FORCES!!!!
...
vector <double> OBABO_simu_MH(const params param0, const size_t N, const double h, const double T, const double gamma, const vector <double>& Xdata, const double sig1, 
   ...
   params theta_curr = param0; 
	forces force_curr = get_noisy_force_GM(T, theta_curr, sig1, sig2, a1, a2, Xdata);	    // HERE FORCES!!!!
...
forces get_noisy_force_GM(const double T, const params& theta, const double sig1, const double sig2, const double a1, const double a2, 
	...
	double P, e1, e2, x;	
	for(int i=0; i<Xdata.size(); ++i){
			P = likelihood_GM(theta, sig1, sig2, a1, a2, Xdata[i]);
			x = Xdata[i];			
	...
	F.fmu1 *= T*a1/(sqrt(2*PI)*sig1);
	F.fmu2 *= T*a2/(sqrt(2*PI)*sig2);
