...
name = "/home/rene/PhD/Research/Code/Integrators/GM/mus_"   # file name head to read
...
n = int(5e5) # average over last n values
ndist = 10000 # plot any n timestamps
...
hs = ["0.001", "0.001", "0.010", "0.100"]
files_h = ["OBABO_h"+h for h in hs]
avgs = ["_72avg", "_36avg", "_5avg", "_1avg"]
files = [files_h[i] + avgs[i] for i in range(0,4)]
labels = [r"OBABO, $h=0.001$, 72 runs", r"OBABO, $h=0.001$, 36 runs", r"OBABO, $h=0.01$, 5 runs", r"OBABO, $h=0.1$, 1 run"]
...
ax[0].set_xlabel(r"$N_{{OBABO}}$")
ax[0].set_ylabel(r"Error")
ax[1].set_xlabel(r"$N_{{OBABO}}$")
fig.suptitle(title)
ax[0].legend()
